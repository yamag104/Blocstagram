            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (storedMediaItems.count > 0) {
                        ...
                        // if download did not finish by the time files are saved, it'll download any images that are nil
                    } else {
        ...
        [self.instagramOperationManager GET:mediaItem.mediaURL.absoluteString
                                 parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                     ...
                                     if ([error.domain isEqualToString:NSURLErrorDomain]) {
                                         ...
                                         if (error.code == NSURLErrorTimedOut || error.code == NSURLErrorCancelled || error.code == NSURLErrorCannotConnectToHost ||
                                             error.code == NSURLErrorNetworkConnectionLost ||
                                             error.code == NSURLErrorNetworkConnectionLost ||
                                             error.code == NSURLErrorNotConnectedToInternet ||
                                             error.code == NSURLErrorInternationalRoamingOff ||
                                             error.code == kCFURLErrorInternationalRoamingOff ||
                                             error.code == kCFURLErrorCallIsActive || error.code == kCFURLErrorDataNotAllowed || error.code == kCFURLErrorRequestBodyStreamExhausted) {
...
- (void) createOperationManager {
    /* |baseURL| is automatically prepended to any relative URLs we provide later. */
